...
NiceSelect.prototype._renderSelectedItems = function() {
  if (this.multiple) {
    ...
    if(this.config.showSelectedItems || this.config.showSelectedItems || window.getComputedStyle(this.dropdown).width == 'auto' || this.selectedOptions.length < 2){
      ...
      selectedHtml = '<span class="mutiple-active">'+this.selectedOptions.length+' '+this.selectedtext+"</span>";
    }
...
NiceSelect.prototype._onClicked = function(e) {
  e.preventDefault();
    ...
    /*NEW*/
    else if((e.target.classList.contains('multiple-options') && this.multiple) || (e.target.classList.contains('mutiple-active') && this.multiple) || (e.target.classList.contains('current') && this.multiple))
    {
        removeClass(this.dropdown, "open");
        triggerModalClose(this.el);
    }
